...
- Phase 9A/9B/9C/9D/9E/9F/9H/9J DeskState/live-proof audits for trade decision map, candidate lifecycle trace, active DeskState, Discord watch alert wording, watch-to-plan promotion proof, replay verdicts, HTF FVG decision-zone alert rendering, and post-restart live-observation proof wiring.
...
- tray/helper parser checks pass for `QuantDeskSupervisorTray.ps1` and `Open-QuantDesk-EvidenceSummary.ps1`;
...
For the Phase 17 live scanner/Discord observation wrapper, run:

```bash
npm run supervisor:live-observation-signoff -- --trade-date <yyyy-mm-dd> --instrument MES --session <morning|lunch|evening> --since-recorded-at <scanner-restart-iso> --json
```

This writes `logs/supervisor/live-observation-signoff/live-observation-signoff-<trade-date>-<instrument>-<session>.json` and combines supervisor Phase 6 signoff, live observer Discord signoff, Phase 4/5 failure counts, HTF routing evidence, and the local evidence-summary readout. It is read-only: it does not post Discord, start scanner services, write Supabase, change scanner state, change trading logic, change `canExecute`, or change entries/stops/targets. Use `--require-evidence-summary` only for end-of-day signoff when the evidence bundle is expected to exist.

To include that same Phase 17 live-observation proof in the real-tape workflow loopback, run:
...
For posted Discord card artifact signoff, run:

```bash
npm run workflow:loopback -- --real-tapes --discord-card-signoff --trade-date=<yyyy-mm-dd> --instrument=MES --session=<morning|lunch|evening> --json
```
...
- Phase 9H HTF FVG decision-zone alert audit reports `pass`, verifies the Discord-rendered FVG decision-zone block includes line, why, hold, fold, no-chase, parent reaction, cascade, watch-only/no-priced-stop wording, and no-authority-change boundaries without changing gates.
- Discord Desk Play posts must use the approved ticket contract: `Primary`, `HTF/HTF Context`, `Line in the Sand`, `Trigger`, `Trade Plan`, `Invalid`, `Status`, and `Chart`. Full plans must include priced `Entry`, `Stop`, `Protected 5M swing`, `T1`, and `T2`; otherwise they must render as `WATCH ONLY` with no priced stop.
- Phase 9J live-observation proof audit reports `pass`, verifies the post-restart live-observation signoff command, loopback flag, read-only authority boundary, scanner-restart timestamp filter, runbook commands, and Phase 9H precondition are wired before live evidence is accepted.
