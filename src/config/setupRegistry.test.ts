...
for (const sessionType of ['morning', 'lunch', 'evening', 'replay_morning', 'replay_lunch'] as const) {
  assert.deepEqual(getPrimarySetupRegistry(sessionType).map((entry) => entry.setupType), expectedSetupTypes, `${sessionType} primary registry must contain the five approved models`);
  assert.deepEqual(getAllowedSetupRegistry(sessionType).map((entry) => entry.setupType), expectedSetupTypes, `${sessionType} allowed registry must contain the five approved models`);
...
assert.deepEqual(SETUP_REGISTRY.map((entry) => entry.setupType), expectedSetupTypes, 'setup registry must contain only the five approved model entries');
assert.deepEqual(REGISTERED_SETUP_TYPES, expectedSetupTypes, 'registry must register only the five approved setup types');
assert.equal(APPROVED_SETUP_TYPES, REGISTERED_SETUP_TYPES, 'approved setup export remains a compatibility alias');

console.log('setupRegistry five-model contract verified');
