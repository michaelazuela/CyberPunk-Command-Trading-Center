...
function historicalReviewCandidate(overrides: Partial<SetupCandidate> = {}): SetupCandidate {
  return {
    ...
    scenarioLabel: 'no installed model path LONG',
    direction: 'LONG',
...
const wideCandidate = historicalReviewCandidate();
...
const displayScore = scoreConditionalCandidateRiskForDisplay(historicalReviewCandidate({
...
assert.equal(inferPriceExtended(historicalReviewCandidate({
  nextAction: 'Manual decision only. Do not chase the reclaim candle.',
})), true);

const insideLimitCandidate = historicalReviewCandidate({
  entry: 7593,
  stop: 7588.75,
  target1: 7602,
  target2: 7605,
