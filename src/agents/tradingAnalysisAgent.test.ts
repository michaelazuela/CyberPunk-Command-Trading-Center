...
const report = buildWeeklyTradingAnalysisReport({
  ...
  researchBackfillReports: [{
    ...
    conceptReports: [{
      ...
      dataGaps: [],
      classificationCounts: { model1Overlap: 0, historicalReversalOverlap: 0, advisoryOnly: 3 },
    ...
    approvedModelOverlap: { model1: 0, historicalReview: 0, total: 0 },
  }],
  researchNotes: [{
...
assert.ok(report.discordMessage.includes([
  ...
  '  Taxonomy: If historical reversal pattern exists, evaluate through existing no installed model path; otherwise keep as advisory research.',
...
assert.ok(report.discordMessage.includes([
  ...
  '  Taxonomy: If no installed model path gates pass, classify through no installed models; otherwise keep as advisory research.',
...
const oversizedNewsletter = buildWeeklyTradingAnalysisReport({
  ...
  researchBackfillReports: [{
    ...
    conceptReports: Array.from({ length: 12 }, (_, index) => ({
      ...
      dataGaps: [],
      classificationCounts: { model1Overlap: 0, historicalReversalOverlap: 0, advisoryOnly: 50 },
...
writeFileSync(join(auditDir, 'scanner.json'), JSON.stringify({
  ...
  state: 'Conditional',
  discordSent: true,
  candidates: [{ setupType: 'historicalReview', direction: 'LONG', executionStatus: 'Conditional' }],
...
const normalizedTradeAudit = normalizeScannerAuditRecord({
  ...
  instrument: 'MES',
  state: 'Executable',
  candidates: [{ setupType: 'historicalReview', direction: 'LONG', executionStatus: 'Executable' }],
  attachments: { chartMarkup: 'chart.png' },
}, join(auditDir, 'scanner.json'));
assert.equal(normalizedTradeAudit.alertType, 'trade');
assert.equal(normalizedTradeAudit.candidateSetupType, 'historicalReview');
...
assert.ok(collected.researchNotes?.some((note) => note.candidateName === 'False-Run Liquidity Fade Near Highs Watchlist' && note.taxonomyNote?.includes('no installed model path')));
assert.ok(collected.researchNotes?.some((note) => note.candidateName === 'Time-Window Liquidity Delivery Watchlist' && note.taxonomyNote?.includes('no installed model path')));
assert.ok(collected.researchNotes?.some((note) => note.candidateName === 'Accumulation–Manipulation–Distribution Range Model Watchlist' && note.taxonomyNote?.includes('no installed model path')));
