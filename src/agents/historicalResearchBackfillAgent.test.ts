...
const fixtureInput: HistoricalResearchBackfillInput = {
  ...
  events: [
    ...
    },
    {
      ...
      direction: 'LONG',
      window: '10:00-11:00 NY',
      summary: 'Current approved no installed model path overlap was present in existing records.',
    ...
    },
    {
      ...
      historicalReversalReclaim: true,
      historicalReversalOverlap: true,
    ...
    },
    {
      ...
      direction: 'SHORT',
      window: 'regular_session',
      summary: 'False-run behavior lacked true historical reversal pattern.',
      classification: 'advisory_only',
      failedOrReversed: true,
      warningPatterns: ['No true historical reversal pattern.'],
    },
    {
...
assert.ok(report.fullCandidateEvents.some((event) => event.possiblehistoricalReversalOverlap));
...
assert.equal(report.approvedModelOverlap.historicalReview, 1);
...
assert.equal(falseRun?.approvedModelOverlaps.historicalReview, 1);
