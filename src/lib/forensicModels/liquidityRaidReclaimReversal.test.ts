...
import { detectLiquidityRaidReclaimReversal } from './liquidityRaidReclaimReversal';
...
const longDetected = detectLiquidityRaidReclaimReversal(context({
  liquiditySweeps: [{
    type: 'sweep',
    direction: 'LONG',
    level: 7440,
    sweptLevelLabel: 'prior session low',
    reclaimed: true,
...
const shortDetected = detectLiquidityRaidReclaimReversal(context({
...
const missing = detectLiquidityRaidReclaimReversal(context({}));
assert.equal(missing.detected, false);
assert.ok(missing.missingEvidence.includes('Missing reclaimed raid/sweep fact.'));
...
console.log('liquidity raid reclaim reversal detector contract verified');
