...
export const HUMAN_REVIEW_LABEL_METADATA: Record<string, HumanReviewLabelMetadata> = {
  ...
  historical_mapping_review: {
    label: 'historical_mapping_review',
    displayName: 'no installed model path Mapping Review',
    category: 'watchlist',
    formalLedgerEligible: false,
    countsTowardCandidateGates: false,
    meaning: 'Legacy label: queue for human-only mapping review against current no installed model path gates.',
    doesNotMean: DOES_NOT_APPROVE,
    suggestedNextAction: 'add_context_or_collect_more_samples',
    boundary: 'research_only_not_execution_authority',
  },
  historical_reversal_mapping_review: {
    label: 'historical_reversal_mapping_review',
    displayName: 'no installed model path Mapping Review',
    category: 'watchlist',
    formalLedgerEligible: false,
    countsTowardCandidateGates: false,
