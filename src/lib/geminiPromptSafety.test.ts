...
assert.ok(htfRule.includes('Runtime promotion is only through FVG Trading System v1'));
...
assert.equal(/approved .*pathway through SetupType\.(HtfDrawContinuationAfterRaid|HtfDisplacementMssContinuation|HtfDisplacementFvgContinuation|OpeningDriveFvgContinuation|AfterLunchDriveFvgContinuation|IntradayMssMicroContinuation|FailedPlanReversal)/.test(htfRule), false);
