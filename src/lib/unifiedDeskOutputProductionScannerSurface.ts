...
export function buildUnifiedDeskOutputProductionScannerSurfaceActivation(args: {
  ...
  const expectedRows = checklist.summary.selectedRows;
  const expectedApprovedRows = checklist.summary.selectedRows;
  const expectedRenderedRows = checklist.summary.selectedRows;
  const sourceBlockers = [
    ...
    checklist.summary.selectedRows >= 2 && checklist.summary.selectedRows <= 3 ? null : 'Final readiness must select one morning row, one lunch row, and optional one evening row.',
    checklist.summary.morningRows === 1 ? null : 'Final readiness must select exactly one morning row.',
    checklist.summary.lunchRows === 1 ? null : 'Final readiness must select exactly one lunch row.',
    (checklist.summary.eveningRows || 0) <= 1 ? null : 'Final readiness must select no more than one evening row.',
  ...
  const rowBlockers = [
    ...
    rows.filter((row) => row.session === 'morning').length === 1 ? null : 'Production surface must have exactly one morning row.',
