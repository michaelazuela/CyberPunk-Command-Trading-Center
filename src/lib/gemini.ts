...
async function superAgent(imageData: ChartImagePayload, settings?: AISettings, previousAnalysis?: any, historicalTrades?: Trade[], modelOverride?: string, routeName?: string, midnightOpenOverride?: string, ragContextStr?: string, dailyInstrument?: string, accountEquity: number = 5000, riskPercent: number = 0.02) {
  ...
  const prompt = `
    ...
    The active live scanner model surface is SetupType.FvgTradingSystemV1 / FVG_TRADING_SYSTEM_V1. There is no active FVG Trading System v2 unless a future code, guard, and test change explicitly adds one. Gemini must extract FVG/imbalance, liquidity sweep, reclaim, MSS, displacement, protected-structure, obstacle, and target-context facts only. Historical setup names are aliases/context only: HtfDrawContinuationAfterRaid, HtfDisplacementMssContinuation, HtfDisplacementFvgContinuation, OpeningDriveFvgContinuation, AfterLunchDriveFvgContinuation, IntradayMssMicroContinuation, FailedPlanReversal, TurtleSoup, and SweepMssFvgRetrace are not approved live setup routes. Narrative fallback cannot create HtfDrawContinuationAfterRaid, HtfDisplacementMssContinuation, HtfDisplacementFvgContinuation, OpeningDriveFvgContinuation, AfterLunchDriveFvgContinuation, IntradayMssMicroContinuation, FailedPlanReversal, TurtleSoup, or SweepMssFvgRetrace candidates. Runtime promotion is only through FVG Trading System v1 and still requires HTF/15M story first, a valid same-direction 15M parent FVG or battle zone, completed 5M confirmation, nearest protected 5M structure stop, app-owned T1/T2 from actual risk, target/obstacle path, invalidation, session gates, deterministic final pipeline approval, and canExecute. 5M MSS may raise confidence for this FVG continuation context, but must not be invented when structured OHLC does not confirm it. Risk above the standard cap is advisory only and must be displayed with "Risk exceeds standard limit. Human final decision required." It may not bypass deterministic entry, stop, target, session, screenshot-quality, final-pipeline, or canExecute gates.
