...
async function superAgent(imageData: ChartImagePayload, settings?: AISettings, previousAnalysis?: any, historicalTrades?: Trade[], modelOverride?: string, routeName?: string, midnightOpenOverride?: string, ragContextStr?: string, dailyInstrument?: string, accountEquity: number = 5000, riskPercent: number = 0.02) {
  ...
  const prompt = `
    ...
    [DESK PLAN MANAGEMENT RULE]
    When the app/scanner DeskState or structured target context says a play is delivering into an HTF/session target, obstacle, liquidity pool, or reaction level, the desk plan must include that management warning as part of the plan narrative:
    - Name the HTF/session target/reaction level.
    ...
    ## Live FVG Trading System v1 Boundary

    The active live scanner model surface is the SetupType.FvgTradingSystemV1 / FVG_TRADING_SYSTEM_V1 model family. There is no active FVG Trading System v2 unless a future code, guard, and test change explicitly adds one. Gemini must extract FVG/imbalance, liquidity sweep, reclaim, MSS, displacement, protected-structure, obstacle, and target-context facts only. Runtime promotion may come only from app-owned deterministic scanner routes in the FVG_TRADING_SYSTEM_V1 family: FvgTradingSystemV1, OpeningDriveFvgContinuation, AfterLunchDriveFvgContinuation, IntradayMssMicroContinuation, and DefendedBattleZoneContinuation. These continuation routes are human-review/decision-support routes only unless the deterministic final pipeline separately approves canExecute. Historical setup names are aliases/context only: HtfDrawContinuationAfterRaid, HtfDisplacementMssContinuation, HtfDisplacementFvgContinuation, FailedPlanReversal, TurtleSoup, and SweepMssFvgRetrace are not approved live setup routes. Narrative fallback cannot create HtfDrawContinuationAfterRaid, HtfDisplacementMssContinuation, HtfDisplacementFvgContinuation, OpeningDriveFvgContinuation, AfterLunchDriveFvgContinuation, IntradayMssMicroContinuation, DefendedBattleZoneContinuation, FailedPlanReversal, TurtleSoup, or SweepMssFvgRetrace candidates. Runtime promotion still requires HTF/15M story first, a valid same-direction 15M parent FVG or battle zone or approved 15M displacement/MSS continuation context, completed 5M confirmation, nearest protected 5M structure stop, app-owned T1/T2 from actual risk, target/obstacle path, invalidation, session gates, deterministic final pipeline approval, and canExecute for executable status. 5M MSS may raise confidence for this FVG continuation context, but must not be invented when structured OHLC does not confirm it. Risk above the standard cap is advisory only and must be displayed with "Risk exceeds standard limit. Human final decision required." It may not bypass deterministic entry, stop, target, session, screenshot-quality, final-pipeline, or canExecute gates.
    ...
    Data-limited context cannot create or approve a candidate by itself. HTF context can support a candidate only when live FVG Trading System v1 conditions are met, 5M confirmed MSS is present, and deterministic entry, stop, target, risk, session, model, and canExecute gates remain intact.
    ...
    Narrative fallback may provide notes only. Narrative fallback cannot create historical setup candidates and cannot approve execution.
      ...
      "candidate_trade_plans": [
        {
          "id": "candidate_1",
          "rank": 1,
          "setup_name": "FVG Trading System v1 | FVG/imbalance facts | Liquidity sweep facts | No Trade",
          "rule_category": "string",
