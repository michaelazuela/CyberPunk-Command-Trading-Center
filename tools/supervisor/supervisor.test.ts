...
assert.ok(trayScript.includes('open-research-status'));
assert.ok(trayScript.includes('Stop Supervisor Services'));
assert.ok(supervisorStopScript.includes('supervisor:stop'));
assert.equal(supervisorStopScript.includes('quant-desk:stop-all'), false);
assert.ok(trayScript.includes('npm run quant-desk:maintenance:off'));
...
fs.writeFileSync(statePath, JSON.stringify({
  sent: {
    '2026-06-04|MES|morning|LONG|historicalReview|7556.5|Approved': {
      state: 'Approved',
      confidence: 96,
  ...
  alertDeliveries: {
    '2026-06-04|MES|morning|LONG|historicalReview|7556.5|Approved': {
      alertKey: '2026-06-04|MES|morning|LONG|historicalReview|7556.5|Approved',
...
fs.writeFileSync(dryRunStatePath, JSON.stringify({
  ...
  alertDeliveries: {
    ...
    '2026-06-04|MES|morning|LONG|historicalReview|dry-run|Missed': {
      alertKey: '2026-06-04|MES|morning|LONG|historicalReview|dry-run|Missed',
...
fs.writeFileSync(staleHistoricalDeliveryStatePath, JSON.stringify({
  ...
  alertDeliveries: {
    '2026-06-18|MES|evening|SHORT|historicalReview|7575|Missed': {
      alertKey: '2026-06-18|MES|evening|SHORT|historicalReview|7575|Missed',
...
assert.equal(staleHistoricalDeliveryReport.lastHistoricalDelivery?.alertKey, '2026-06-18|MES|evening|SHORT|historicalReview|7575|Missed');
