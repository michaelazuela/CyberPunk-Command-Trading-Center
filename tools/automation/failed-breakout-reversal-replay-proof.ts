...
import { detectFailedBreakoutReversal } from '../../src/lib/forensicModels/failedBreakoutReversal';
import type {
  ChartCandleFact,
  ChartContext,
...
function failedBreakFacts(candles: ChartCandleFact[]): {
  ...
  for (let index = 3; index < candles.length; index += 1) {
    ...
    const priorHigh = Math.max(...lookback.map((item) => item.high ?? Number.NEGATIVE_INFINITY));
    const priorLow = Math.min(...lookback.map((item) => item.low ?? Number.POSITIVE_INFINITY));
    if (!Number.isFinite(candle.high) || !Number.isFinite(candle.low) || !Number.isFinite(candle.close)) continue;

    if (Number.isFinite(priorLow) && candle.low < priorLow && candle.close > priorLow) {
      failedBreakEvents.push({
        ...
        confidence: 'High',
        evidence: 'Failed downside breakout below recent 5M swing low.',
      });
      reclaimEvents.push({
        direction: 'LONG',
        ...
        candleIndex: candle.index,
        confidence: 'High',
        evidence: 'Completed 5M close reclaimed the failed downside breakout level.',
      });
    }

    if (Number.isFinite(priorHigh) && candle.high > priorHigh && candle.close < priorHigh) {
      failedBreakEvents.push({
        direction: 'SHORT',
        ...
        candleIndex: candle.index,
        confidence: 'High',
        evidence: 'Failed upside breakout above recent 5M swing high.',
      });
      reclaimEvents.push({
        direction: 'SHORT',
        ...
        candleIndex: candle.index,
        confidence: 'High',
        evidence: 'Completed 5M close reclaimed below the failed upside breakout level.',
      });
    }
  }
  return { failedBreakEvents, reclaimEvents };
}
...
function contextFor(args: {
  ...
  const facts = failedBreakFacts(fiveMinute.candles);
...
export function buildFailedBreakoutReversalReplayProof(args: Args) {
  ...
  for (const session of args.sessions) {
    ...
    for (const currentBar of evaluationBars) {
      ...
      contextsEvaluated += 1;
      const detection = detectFailedBreakoutReversal(context);
      if (
        detection.detected &&
        detection.direction &&
        ...
        rows.push({
          ...
          failedLevel: detection.failedLevel,
          failedLevelLabel: detection.failedLevelLabel,
  ...
  return {
    reportType: 'failed_breakout_reversal_replay_proof',
...
function writeReport(report: ReturnType<typeof buildFailedBreakoutReversalReplayProof>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(
    outDir,
    `failed-breakout-reversal-replay-proof-${report.source.instrument}-${report.source.startDate}-to-${report.source.endDate}-${Date.now()}.json`
  );
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'failed-breakout-reversal-replay-proof.ts') {
  const args = parseArgs();
  const report = buildFailedBreakoutReversalReplayProof(args);
