...
import {
  buildLiquidityRaidReclaimPdfWindowFilter,
  extractProfitablePdfTradesFromText,
} from './liquidity-raid-reclaim-reversal-pdf-window-filter';
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'liq-pdf-filter-'));
...
const report = buildLiquidityRaidReclaimPdfWindowFilter({
  pdfPath: null,
  pdfTextPath,
...
console.log('liquidity raid reclaim reversal PDF-window filter verified');
