...
export interface RawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport {
  reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_long_full_day_scanner_field_selection_simulation';
  generatedAt: string;
  status: 'pass' | 'fail';
  authority: {
    readOnly: true;
...
function authority(): RawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function buildMarkdown(report: Omit<RawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport, 'markdown'>): string {
  return [
    '# Raw OHLC Scanner Artifact Sweep Lunch LONG Full-Day Scanner Field Selection Simulation',
    '',
...
export function buildRawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport(args: {
  reportDir?: string;
  scannerFieldMinerPath?: string | null;
  scannerFieldMiner?: ScannerFieldMinerReport | null;
}, generatedAt = new Date().toISOString()): RawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport {
  ...
  const base: Omit<RawOhlcScannerArtifactSweepLunchLongFullDayScannerFieldSelectionSimulationReport, 'markdown'> = {
    reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_long_full_day_scanner_field_selection_simulation',
    generatedAt,
...
function runCli(): void {
  ...
  const outPath = path.join(reportDir, `raw-ohlc-scanner-artifact-sweep-lunch-long-full-day-scanner-field-selection-simulation-${Date.now()}.json`);
