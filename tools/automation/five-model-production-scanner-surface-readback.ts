...
export function buildFiveModelProductionScannerSurfaceReadbackReport(args: {
  ...
  const blockers = [
    ...
    rows.length === 18 || rows.length === 19 ? null : `Runtime readback expected 18 or 19 rows and found ${rows.length}.`,
    surface?.summary.approvedDeskPlanRows === 5 ? null : 'Runtime readback expected 5 Approved Desk Plan rows.',
    surface?.summary.formingDeskReadRows === rows.length - 5 ? null : 'Runtime readback Forming Desk Read row count does not match selected rows.',
    ...
    surface?.summary.eveningRows === 0 || surface?.summary.eveningRows === 1 ? null : 'Runtime readback expected 0 or 1 evening rows.',
