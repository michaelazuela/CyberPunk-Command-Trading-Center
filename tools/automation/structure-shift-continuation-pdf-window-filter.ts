...
export function buildStructureShiftContinuationPdfWindowFilter(args: Args) {
  ...
  return {
    reportType: 'structure_shift_continuation_pdf_window_filter',
...
function writeReport(report: ReturnType<typeof buildStructureShiftContinuationPdfWindowFilter>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(
    outDir,
    `structure-shift-continuation-pdf-window-filter-${report.source.startDate}-to-${report.source.endDate}-${Date.now()}.json`
  );
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'structure-shift-continuation-pdf-window-filter.ts') {
  const args = parseArgs();
  const report = buildStructureShiftContinuationPdfWindowFilter(args);
