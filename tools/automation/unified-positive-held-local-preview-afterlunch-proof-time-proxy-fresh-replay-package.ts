...
function buildMarkdown(report: Omit<UnifiedPositiveHeldLocalPreviewReplayPackageReport, 'markdown'>): string {
  return [
    '# Unified Positive Held-Local Preview AfterLunch Proof-Time Proxy Fresh Replay Package',
    '',
    `Status: ${report.status}`,
    '',
    'Authority: local-only read-only replay package rebuilt from saved AfterLunch proof-time proxy tickets and saved source replay package rows. It does not run setupScanner, post Discord, write Supabase, read live bridge data, install rank behavior, loosen canExecute, or change entry/stop/target/risk math.',
    '',
    '## Summary',
...
export function parseUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageArgs(args = process.argv.slice(2)): CliOptions {
  ...
  return {
    validationPackage: readFlag(args, '--validation-package') ||
      latestMatchingFile(outDir, 'unified-positive-held-local-preview-afterlunch-proof-time-proxy-validation-package-'),
    sourceReplayPackages: explicitSourcePackages.length
      ? explicitSourcePackages
      : matchingFiles(outDir, /^(?:raw-ohlc-scanner-artifact-replay-package|unified-positive-held-local-preview-replay-package)-\d+\.json$/),
    outDir,
    json: args.includes('--json'),
  };
}

export function buildUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageReport(args: {
  reportDir: string;
  ...
  const requestedTickets = new Set((args.validationPackage?.rows || []).map((row) => row.ticketId));
  ...
  const blockers = [
    !args.validationPackagePath ? 'missing AfterLunch proof-time proxy validation package path' : null,
    !args.validationPackage ? 'missing AfterLunch proof-time proxy validation package' : null,
    args.validationPackage && args.validationPackage.status !== 'pass' ? `AfterLunch proof-time proxy validation package status ${args.validationPackage.status}` : null,
    ...
    requestedTickets.size === 0 ? 'AfterLunch proof-time proxy validation package has no rows' : null,
    rows.length === 0 ? 'no AfterLunch proof-time proxy tickets matched source replay package rows' : null,
  ...
  const base: Omit<UnifiedPositiveHeldLocalPreviewReplayPackageReport, 'markdown'> = {
    ...
    recommendations: blockers.length
      ? ['Do not run fresh outcome replay until every AfterLunch proxy ticket maps to a ready saved source replay row.']
      : ['Run the existing read-only OHLC outcome replay over this package, then compare fresh outcomes against the proxy manifest before any scanner-visible proposal.'],
  };
  return { ...base, markdown: buildMarkdown(base) };
}

export function writeUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageReport(
  report: UnifiedPositiveHeldLocalPreviewReplayPackageReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `unified-positive-held-local-preview-afterlunch-proof-time-proxy-fresh-replay-package-${Date.now()}`;
...
export function runUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageCli(args = process.argv.slice(2)): void {
  const options = parseUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageArgs(args);
  const sourceReplayPackages = options.sourceReplayPackages.map((filePath) => readJson<UnifiedPositiveHeldLocalPreviewReplayPackageReport>(filePath));
  const report = buildUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageReport({
    ...
    validationPackage: options.validationPackage && fs.existsSync(options.validationPackage)
      ? readJson<UnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyValidationPackageReport>(options.validationPackage)
      : null,
    sourceReplayPackagePaths: options.sourceReplayPackages,
    sourceReplayPackages,
  });
  const paths = writeUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageReport(report, options.outDir);
  if (options.json) {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runUnifiedPositiveHeldLocalPreviewAfterLunchProofTimeProxyFreshReplayPackageCli();
}
