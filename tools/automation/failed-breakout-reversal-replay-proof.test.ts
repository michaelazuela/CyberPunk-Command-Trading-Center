...
import { buildFailedBreakoutReversalReplayProof } from './failed-breakout-reversal-replay-proof';

const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'failed-breakout-reversal-proof-'));
...
const bars5m = [
  ...
  { time: '2026-06-08T09:35:00', open: 100.5, high: 102, low: 100.25, close: 101.5 },
];
...
const report = buildFailedBreakoutReversalReplayProof({
  marketBarsJson,
...
assert.equal(report.rows[0]?.failedLevel, 99);
assert.equal(report.rows[0]?.failedLevelLabel, 'recent 5M swing low');
assert.equal(report.rows[0]?.proofTime, '2026-06-08T09:30:00');

console.log('failed breakout reversal replay proof verified');
