...
assert.equal(bearish.htfLiquidityDrawState.htfContextSufficiency.overallStatus, 'sufficient');
assert.equal(bearish.htfLiquidityDrawState.classificationReliability, 'structural');
assert.equal(bearish.htfLiquidityDrawState.htfContextDataLimited, false);
...
assert.ok(bearishMarkdown.includes('## HTF Context Sufficiency'));
assert.ok(bearishMarkdown.includes('Status: sufficient'));
assert.ok(bearishMarkdown.includes('Reliability: structural'));
assert.ok(bearishMarkdown.includes('HTF Usage: structural confirmation allowed'));
assert.ok(bearishMarkdown.includes('Candidate Promotion: allowed only when approved pathway conditions and deterministic gates are satisfied'));
