...
export function buildLiquidityRaidReclaimPdfWindowFilter(args: Args) {
  ...
  return {
    reportType: 'liquidity_raid_reclaim_reversal_pdf_window_filter',
...
function writeReport(report: ReturnType<typeof buildLiquidityRaidReclaimPdfWindowFilter>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(
    outDir,
    `liquidity-raid-reclaim-reversal-pdf-window-filter-${report.source.startDate}-to-${report.source.endDate}-${Date.now()}.json`
  );
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'liquidity-raid-reclaim-reversal-pdf-window-filter.ts') {
  const args = parseArgs();
  const report = buildLiquidityRaidReclaimPdfWindowFilter(args);
