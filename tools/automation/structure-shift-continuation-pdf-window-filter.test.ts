...
import {
  buildStructureShiftContinuationPdfWindowFilter,
  extractProfitablePdfTradesFromText,
} from './structure-shift-continuation-pdf-window-filter';
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'structure-shift-pdf-filter-'));
...
const report = buildStructureShiftContinuationPdfWindowFilter({
  pdfPath: null,
  pdfTextPath,
...
console.log('structure shift continuation PDF-window filter verified');
