...
import {
  buildFailedBreakoutReversalPdfWindowFilter,
  extractProfitablePdfTradesFromText,
} from './failed-breakout-reversal-pdf-window-filter';
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'failed-breakout-pdf-filter-'));
  ...
  rows: [
    {
      ...
      riskPoints: 2,
      failedLevel: 7438.75,
      failedLevelLabel: 'recent 5M swing low',
      htfContext: 'support',
      evidence: [],
    },
    {
      ...
      riskPoints: 5,
      failedLevel: 7390,
      failedLevelLabel: 'recent 5M swing high',
      htfContext: 'conflict',
      evidence: [],
    },
  ],
...
const report = buildFailedBreakoutReversalPdfWindowFilter({
  pdfPath: null,
  pdfTextPath,
...
assert.equal(report.matches[0]?.minutesBeforeEntry, 13);
assert.equal(report.matches[1]?.matched, false);

console.log('failed breakout reversal PDF-window filter verified');
