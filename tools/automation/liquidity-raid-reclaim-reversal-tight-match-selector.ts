...
export function buildLiquidityRaidReclaimTightMatchSelector(args: Args) {
...
function writeReport(report: ReturnType<typeof buildLiquidityRaidReclaimTightMatchSelector>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(outDir, `liquidity-raid-reclaim-reversal-tight-match-selector-${Date.now()}.json`);
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'liquidity-raid-reclaim-reversal-tight-match-selector.ts') {
  const args = parseArgs();
  const report = buildLiquidityRaidReclaimTightMatchSelector(args);
