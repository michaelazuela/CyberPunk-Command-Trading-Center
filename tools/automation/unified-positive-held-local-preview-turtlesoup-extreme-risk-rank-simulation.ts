...
export interface UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport {
  reportType: 'unified_positive_held_local_preview_turtlesoup_extreme_risk_rank_simulation';
  generatedAt: string;
  status: 'pass' | 'fail';
  authority: {
    readOnly: true;
...
function authority(): UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function recommendation(args: {
  ...
  penalizedTopAfterSlates: number;
  penalizedTopBeforeSlates: number;
}): UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport['summary']['recommendation'] {
...
function buildMarkdown(report: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport, 'markdown'>): string {
  return [
    '# Unified Positive Held-Local Preview TurtleSoup Extreme-Risk Rank Simulation',
    '',
    `Status: ${report.status}`,
    '',
    'Authority: local-only read-only rank simulation. It does not install rank penalties, remove TurtleSoup, hard-block a setup, post Discord, write Supabase, read live bridge data, run setupScanner, change canExecute, or change entry/stop/target/risk math.',
    '',
    '## Summary',
    `- Joined rows: ${report.summary.joinedRows}.`,
    `- Slates: ${report.summary.slates}.`,
    `- Penalized TurtleSoup risk > 15 rows: ${report.summary.penalizedRows}.`,
...
export function buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport(args: {
  reportDir: string;
  installedScoreComparisonPath: string | null;
...
}, generatedAt = new Date().toISOString()): UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport {
  ...
  const blockers = [
    ...
    args.separatorDiagnosticReport &&
      !args.separatorDiagnosticReport.topCautionBuckets.some((row) => row.key === 'TurtleSoup|risk_extreme_over_15')
      ? 'separator diagnostic did not identify TurtleSoup|risk_extreme_over_15 caution bucket'
      : null,
  ...
  const base: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport, 'markdown'> = {
    reportType: 'unified_positive_held_local_preview_turtlesoup_extreme_risk_rank_simulation',
    generatedAt,
    ...
    recommendations: blockers.length
      ...
      : rec === 'continue_research_only_extreme_risk_penalty'
        ? [
          'Continue research only with a TurtleSoup risk-width rank penalty candidate; do not install live behavior yet.',
          'Review any false winner demotion manually before proposing scanner-visible behavior.',
        ]
        : rec === 'review_note_only'
          ? ['Use TurtleSoup extreme-risk as a review-note hypothesis only; rank effect is not strong enough.']
          : ['Reject TurtleSoup extreme-risk rank penalty for now.'],
  };
  return { ...base, markdown: buildMarkdown(base) };
}

export function writeUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport(
  report: UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `unified-positive-held-local-preview-turtlesoup-extreme-risk-rank-simulation-${Date.now()}`;
...
export function runUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationCli(args = process.argv.slice(2)): void {
  ...
  const report = buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport({
    reportDir: outDir,
  ...
  const paths = writeUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport(report, outDir);
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    runUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationCli();
  } catch (error) {
