...
export function buildRaidFailureDisplacementPdfWindowFilter(args: Args) {
  ...
  return {
    reportType: 'raid_failure_displacement_reversal_pdf_window_filter',
...
function writeReport(report: ReturnType<typeof buildRaidFailureDisplacementPdfWindowFilter>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(
    outDir,
    `raid-failure-displacement-reversal-pdf-window-filter-${report.source.startDate}-to-${report.source.endDate}-${Date.now()}.json`
  );
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'raid-failure-displacement-reversal-pdf-window-filter.ts') {
  const args = parseArgs();
  const report = buildRaidFailureDisplacementPdfWindowFilter(args);
