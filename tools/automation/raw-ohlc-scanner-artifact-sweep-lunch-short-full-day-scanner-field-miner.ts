...
export interface RawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport {
  reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_short_full_day_scanner_field_miner';
  generatedAt: string;
  status: 'pass' | 'fail';
  authority: {
    readOnly: true;
  ...
  for (const artifactPath of artifactPaths) {
    ...
    for (const event of events(artifact)) {
      ...
      statuses.forEach((status, index) => {
        ...
        const direction = statusRow.direction === 'SHORT' || statusRow.direction === 'SHORT' ? statusRow.direction : '';
...
function authority(): RawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function buildMarkdown(report: Omit<RawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport, 'markdown'>): string {
  return [
    '# Raw OHLC Scanner Artifact Sweep lunch SHORT Full-Day Scanner Field Miner',
    '',
...
export function buildRawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport(args: {
  reportDir?: string;
  fullDayRollupPath?: string | null;
  replayPackagePath?: string | null;
  ...
  joinedRows?: JoinedRow[];
}, generatedAt = new Date().toISOString()): RawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport {
  ...
  const blockers = [
    ...
    filteredOutcomeRows.length === 0 && !args.joinedRows ? 'no SweepMssFvgRetrace lunch SHORT full-day rows found' : null,
    joinedRows.length === 0 ? 'no SweepMssFvgRetrace lunch SHORT scanner rows joined to full-day outcomes' : null,
  ].filter((item): item is string => Boolean(item));
  const base: Omit<RawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport, 'markdown'> = {
    reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_short_full_day_scanner_field_miner',
    generatedAt,
...
function runCli(): void {
  ...
  const report = buildRawOhlcScannerArtifactSweepLunchShortFullDayScannerFieldMinerReport({
    reportDir,
    fullDayRollupPath: readFlag(args, '--full-day-rollup') || undefined,
    replayPackagePath: readFlag(args, '--replay-package') || undefined,
  });
  fs.mkdirSync(reportDir, { recursive: true });
  const outPath = path.join(reportDir, `raw-ohlc-scanner-artifact-sweep-lunch-short-full-day-scanner-field-miner-${Date.now()}.json`);
