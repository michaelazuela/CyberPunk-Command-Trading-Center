...
const placementSimulation = {
  ...
  rows: [{
    ...
    session: 'morning',
    direction: 'LONG',
    proposedNote: 'historicalReview long remains review-only: the liquidity-raid idea is visible, but this cluster lacks full plan-level proof. Require fresh completed 5M entry, protected stop, invalidation, and app targets before treating it as actionable.',
  ...
  recommendations: [],
  markdown: '',
} satisfies UnifiedPositiveHeldLocalPreviewhistoricalReviewReviewNotePlacementSimulationReport;
...
assert.equal(placementReport.source.historicalReviewReviewNotePlacementSimulationPath, 'placement.json');
