...
import {
  buildUnifiedPositiveHeldLocalPreviewTurtleSoupRankPenaltyValidationReport,
...
const actionProbeReport: UnifiedPositiveHeldLocalPreviewTurtleSoupActionProbeReport = {
  reportType: 'unified_positive_held_local_preview_turtlesoup_action_probe',
  generatedAt: '2026-07-17T00:00:00.000Z',
  status: 'pass',
  authority,
  source: { reportDir: 'diagnostic-reports', turtleSoupReplayPackagePath: 'package.json' },
  assumptions: {
...
const report = buildUnifiedPositiveHeldLocalPreviewTurtleSoupRankPenaltyValidationReport({
  ...
  turtleSoupActionProbeReport: actionProbeReport,
}, '2026-07-17T00:01:00.000Z');

assert.equal(report.reportType, 'unified_positive_held_local_preview_turtlesoup_rank_penalty_validation');
...
const missing = buildUnifiedPositiveHeldLocalPreviewTurtleSoupRankPenaltyValidationReport({
  reportDir: 'diagnostic-reports',
  turtleSoupReplayPackagePath: null,
  turtleSoupReplayPackageReport: null,
  turtleSoupActionProbePath: null,
  turtleSoupActionProbeReport: null,
}, '2026-07-17T00:02:00.000Z');

assert.equal(missing.status, 'fail');
assert.ok(missing.blockers.includes('missing TurtleSoup replay package path'));

console.log('unified positive held-local TurtleSoup rank penalty validation verified.');
