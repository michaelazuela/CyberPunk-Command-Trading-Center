...
const report = await buildDeskPlaybookRagWeightingPreviewReport({
  ...
  previewReport,
  ragMemoryRows: [
    { session_type: 'morning', setupType: 'NoInstalledSetup', direction: 'SHORT', trade_result: 'win', pnl_dollars: 250 },
    { session_type: 'morning', setupType: 'NoInstalledSetup', direction: 'SHORT', trade_result: 'win', pnl_dollars: 125 },
    { session_type: 'morning', setupType: 'NoInstalledSetup', direction: 'SHORT', trade_result: 'win', pnl_dollars: 75 },
    { session_type: 'lunch', setupType: 'NoInstalledSetup', direction: 'LONG', trade_result: 'loss', pnl_dollars: -100 },
    { session_type: 'lunch', setupType: 'NoInstalledSetup', direction: 'LONG', trade_result: 'loss', pnl_dollars: -120 },
    { session_type: 'lunch', setupType: 'NoInstalledSetup', direction: 'LONG', trade_result: 'win', pnl_dollars: 50 },
    { session_type: 'morning', plan_source: 'NoInstalledSetup', direction: 'LONG', outcome: 'win', pnl_dollars: 40 },
  ],
...
const boosted = report.tickets.find((ticket) => ticket.primaryModel === 'NoInstalledSetup');
...
const penalized = report.tickets.find((ticket) => ticket.primaryModel === 'NoInstalledSetup');
assert.equal(penalized?.ragAdjustment, 'penalty');
assert.equal(penalized?.weightedDecision, 'watch_caution');

const insufficient = report.tickets.find((ticket) => ticket.primaryModel === 'NoInstalledSetup');
assert.equal(insufficient?.ragAdjustment, 'insufficient_memory');
assert.equal(insufficient?.memoryStats.completedRows, 1);

const noMemory = report.tickets.find((ticket) => ticket.primaryModel === 'NoInstalledSetup');
