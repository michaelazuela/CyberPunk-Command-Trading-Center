...
import type {
  UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport,
} from './unified-positive-held-local-preview-turtlesoup-extreme-risk-rank-simulation';
...
export interface UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport {
...
function authority(): UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function variantSummary(rows: CandidateRow[], variant: typeof VARIANTS[number]): { summary: VariantSummary; changed: ChangedSlate[] } {
  ...
  const penalizes = (row: CandidateRow) => isValidReview(row) && row.setupType === 'TurtleSoup' && variant.matches(row);
...
function buildMarkdown(report: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport, 'markdown'>): string {
  return [
    '# Unified Positive Held-Local Preview TurtleSoup Extreme-Risk Companion Filter',
    '',
...
export function buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport(args: {
  reportDir: string;
  installedScoreComparisonPath: string | null;
  ...
  extremeRiskSimulationPath: string | null;
  extremeRiskSimulationReport: UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport | null;
}, generatedAt = new Date().toISOString()): UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport {
  ...
  const recommendation = blockers.length ? 'reject_missing_source'
    : candidateVariants > 0 ? 'fresh_validate_candidate_variant'
      : 'do_not_install_extreme_risk_penalty';
  const base: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport, 'markdown'> = {
    reportType: 'unified_positive_held_local_preview_turtlesoup_extreme_risk_companion_filter',
    generatedAt,
    ...
    recommendations: blockers.length
      ...
      : candidateVariants > 0
        ...
        : [
          'Do not install a TurtleSoup extreme-risk rank penalty from this evidence.',
          'Treat wide-risk TurtleSoup as a human-review caution note until a valid replacement-positive separator is proven.',
        ],
  };
  return { ...base, markdown: buildMarkdown(base) };
}

export function writeUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport(
  report: UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `unified-positive-held-local-preview-turtlesoup-extreme-risk-companion-filter-${Date.now()}`;
...
export function runUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterCli(args = process.argv.slice(2)): void {
  ...
  const extremeRiskSimulationPath = readFlag(args, '--extreme-risk-simulation') ||
    latestMatchingFile(outDir, /^unified-positive-held-local-preview-turtlesoup-extreme-risk-rank-simulation-\d+\.json$/);
  const report = buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport({
    reportDir: outDir,
    ...
    extremeRiskSimulationReport: extremeRiskSimulationPath && fs.existsSync(extremeRiskSimulationPath)
      ? readJson<UnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport>(extremeRiskSimulationPath)
      : null,
  });
  const paths = writeUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterReport(report, outDir);
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    runUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskCompanionFilterCli();
  } catch (error) {
