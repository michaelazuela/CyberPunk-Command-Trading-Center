...
interface FeatureStatRow {
  feature: string;
  value: string;
  verdict: 'positive_candidate' | 'negative_candidate' | 'mixed_or_too_small';
}
...
export interface RawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport {
  reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_long_full_day_stable_selection_simulation';
  generatedAt: string;
  status: 'pass' | 'fail';
  authority: {
    readOnly: true;
...
function authority(): RawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function buildMarkdown(report: Omit<RawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport, 'markdown'>): string {
  return [
    '# Raw OHLC Scanner Artifact Sweep Lunch LONG Full-Day Stable Selection Simulation',
    '',
...
export function buildRawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport(args: {
  reportDir?: string;
  fullDayRollupPath?: string | null;
  stableFieldMinerPath?: string | null;
  fullDayRollup?: FullDayRollupReport | null;
  stableFieldMiner?: StableFieldMinerReport | null;
}, generatedAt = new Date().toISOString()): RawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport {
  ...
  const stableFieldMinerPath = args.stableFieldMinerPath ?? latestMatchingFile(reportDir, 'raw-ohlc-scanner-artifact-sweep-lunch-long-full-day-stable-field-miner-');
  ...
  const blockers = [
    ...
    targetRows.length === 0 ? 'no NoInstalledSetup lunch LONG full-day rows found' : null,
  ...
  const base: Omit<RawOhlcScannerArtifactSweepLunchLongFullDayStableSelectionSimulationReport, 'markdown'> = {
    reportType: 'raw_ohlc_scanner_artifact_sweep_lunch_long_full_day_stable_selection_simulation',
    generatedAt,
...
function runCli(): void {
  ...
  const outPath = path.join(reportDir, `raw-ohlc-scanner-artifact-sweep-lunch-long-full-day-stable-selection-simulation-${Date.now()}.json`);
