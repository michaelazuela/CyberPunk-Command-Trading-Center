...
type DrilldownCluster = UnifiedPositiveHeldLocalPreviewTurtleSoupBlockedReasonDrilldownReport['clusters'][number];
...
export interface UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport {
...
function authority(): UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function proposedNote(cluster: DrilldownCluster): string {
  ...
  return `TurtleSoup ${side} remains review-only: the liquidity-raid idea is visible, but this cluster lacks full plan-level proof. Require fresh completed 5M entry, protected stop, invalidation, and app targets before treating it as actionable.`;
}
...
function buildMarkdown(report: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport, 'markdown'>): string {
  return [
    '# Unified Positive Held-Local Preview TurtleSoup Review Note Wording Probe',
    '',
    `Status: ${report.status}`,
    '',
    'Authority: local-only read-only wording probe. It does not install review notes, suppress tickets, alter ranking, change canExecute, post Discord, write Supabase, read live bridge data, run setupScanner, remove TurtleSoup, or change entry/stop/target/risk rules.',
    '',
    '## Summary',
...
export function buildUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport(args: {
  reportDir: string;
  blockedReasonDrilldownPath: string | null;
  blockedReasonDrilldownReport: UnifiedPositiveHeldLocalPreviewTurtleSoupBlockedReasonDrilldownReport | null;
}, generatedAt = new Date().toISOString()): UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport {
  ...
  const blockers = [
    !args.blockedReasonDrilldownPath ? 'missing TurtleSoup blocked reason drilldown path' : null,
    !args.blockedReasonDrilldownReport ? 'missing TurtleSoup blocked reason drilldown report' : null,
    args.blockedReasonDrilldownReport && args.blockedReasonDrilldownReport.status !== 'pass' ? `TurtleSoup blocked reason drilldown status ${args.blockedReasonDrilldownReport.status}` : null,
    clusters.length === 0 ? 'no TurtleSoup blocked reason clusters found' : null,
  ...
  const base: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport, 'markdown'> = {
    reportType: 'unified_positive_held_local_preview_turtlesoup_review_note_wording_probe',
    generatedAt,
    ...
    recommendations: blockers.length
      ? ['Do not use TurtleSoup review-note wording until the blocked reason drilldown is present and passing.']
      : [
        'Keep this wording as a research-only candidate until a separate approved user-facing wording phase.',
        'Do not suppress TurtleSoup tickets, change ranking, change canExecute, or alter entry/stop/target/risk behavior.',
      ],
  };
  return { ...base, markdown: buildMarkdown(base) };
}

export function writeUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport(
  report: UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `unified-positive-held-local-preview-turtlesoup-review-note-wording-probe-${Date.now()}`;
...
export function runUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeCli(args = process.argv.slice(2)): void {
  const outDir = readFlag(args, '--out-dir') || DEFAULT_REPORT_DIR;
  const blockedReasonDrilldownPath = readFlag(args, '--blocked-reason-drilldown') ||
    latestMatchingFile(outDir, /^unified-positive-held-local-preview-turtlesoup-blocked-reason-drilldown-\d+\.json$/);
  const report = buildUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport({
    ...
    blockedReasonDrilldownReport: blockedReasonDrilldownPath && fs.existsSync(blockedReasonDrilldownPath)
      ? JSON.parse(fs.readFileSync(blockedReasonDrilldownPath, 'utf8')) as UnifiedPositiveHeldLocalPreviewTurtleSoupBlockedReasonDrilldownReport
      : null,
  });
  const paths = writeUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport(report, outDir);
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    runUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeCli();
  } catch (error) {
