...
import {
  buildRaidFailureDisplacementPdfWindowFilter,
  extractProfitablePdfTradesFromText,
} from './raid-failure-displacement-reversal-pdf-window-filter';
...
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'raid-failure-displacement-pdf-filter-'));
  ...
  rows: [
    {
      ...
      riskPoints: 2.25,
      raidLevel: 7440,
      displacementTime: '2026-06-08T10:10:00',
    ...
    },
    {
      ...
      riskPoints: 5,
      raidLevel: 7390,
      displacementTime: '2026-06-09T10:00:00',
      displacementQuality: 'confirmed',
      htfContext: 'support',
      evidence: [],
    },
  ],
...
const report = buildRaidFailureDisplacementPdfWindowFilter({
  pdfPath: null,
  pdfTextPath,
...
console.log('raid failure displacement reversal PDF-window filter verified');
