...
const TARGET_SETUPS = new Set(['TurtleSoup', 'SweepMssFvgRetrace']);
...
function recommendationFor(row: Pick<UnifiedPositiveFresh5mProofRow, 'proofStatus' | 'setupType'>): string {
  ...
  if (row.proofStatus === 'not_in_scope') return 'Not part of the TurtleSoup/Sweep positive proof-capture phase.';
...
function blockersFor(status: FreshProofStatus): string[] {
  ...
  if (status === 'not_in_scope') return ['row not in TurtleSoup/Sweep proof scope'];
