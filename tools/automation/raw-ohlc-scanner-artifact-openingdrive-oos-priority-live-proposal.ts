...
export interface RawOhlcScannerArtifactOpeningDriveOosPriorityLiveProposalReport {
  ...
  proposedBehavior: {
    ...
    eligiblePriorityModels: Array<'NoInstalledSetup' | 'NoInstalledSetup'>;
...
export function buildRawOhlcScannerArtifactOpeningDriveOosPriorityLiveProposalReport(args: {
  ...
  const base: Omit<RawOhlcScannerArtifactOpeningDriveOosPriorityLiveProposalReport, 'markdown'> = {
    ...
    proposedBehavior: {
      ...
      scannerVisibleInstallAllowedNow: false,
      requiredFutureApproval: true,
      rankingIntent: 'When NoInstalledSetup and a clean same-direction NoInstalledSetup or NoInstalledSetup candidate exist at the same proof event, prefer the Sweep/HTF candidate in research ranking before OpeningDrive.',
      eligiblePriorityModels: ['NoInstalledSetup', 'NoInstalledSetup'],
      requiredConditions: [
        'same trade date',
        'same session',
