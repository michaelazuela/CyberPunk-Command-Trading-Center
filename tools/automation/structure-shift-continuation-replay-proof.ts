...
import { detectStructureShiftContinuation } from '../../src/lib/forensicModels/structureShiftContinuation';
import type { ChartCandleFact, ChartContext, MultiTimeframeContext, TimeframeFactSet } from '../../src/types';
...
export function buildStructureShiftContinuationReplayProof(args: Args) {
  ...
  for (const session of args.sessions) {
    ...
    for (const currentBar of evaluationBars) {
      ...
      contextsEvaluated += 1;
      const detection = detectStructureShiftContinuation(context);
      if (
        detection.detected &&
        detection.direction &&
        ...
        rows.push({
          ...
          shiftTime: detection.shiftTime,
          shiftLevel: detection.shiftLevel,
  ...
  return {
    reportType: 'structure_shift_continuation_replay_proof',
...
function writeReport(report: ReturnType<typeof buildStructureShiftContinuationReplayProof>): string {
  const outDir = path.resolve('tools/automation/diagnostic-reports');
  fs.mkdirSync(outDir, { recursive: true });
  const filePath = path.join(
    outDir,
    `structure-shift-continuation-replay-proof-${report.source.instrument}-${report.source.startDate}-to-${report.source.endDate}-${Date.now()}.json`
  );
  fs.writeFileSync(filePath, `${JSON.stringify(report, null, 2)}\n`);
  return filePath;
}

if (process.argv[1] && path.basename(process.argv[1]) === 'structure-shift-continuation-replay-proof.ts') {
  const args = parseArgs();
  const report = buildStructureShiftContinuationReplayProof(args);
