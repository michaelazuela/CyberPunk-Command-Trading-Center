...
export interface RawOhlcScannerArtifactSweepLunchShortFullDayStableFieldMinerReport {
...
function buildFeatureStats(rows: OutcomeRow[]): FeatureStatRow[] {
...
function authority(): RawOhlcScannerArtifactSweepLunchShortFullDayStableFieldMinerReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function buildMarkdown(report: Omit<RawOhlcScannerArtifactSweepLunchShortFullDayStableFieldMinerReport, 'markdown'>): string {
  return [
    '# Raw OHLC Scanner Artifact Sweep Lunch SHORT Full-Day Stable Field Miner',
    '',
...
export function buildRawOhlcScannerArtifactSweepLunchShortFullDayStableFieldMinerReport(args: {
  reportDir?: string;
  fullDayRollupPath?: string | null;
  fullDayRollup?: FullDayRollupReport | null;
}, generatedAt = new Date().toISOString()): RawOhlcScannerArtifactSweepLunchShortFullDayStableFieldMinerReport {
  ...
  const featureStats = buildFeatureStats(targetRows);
...
function runCli(): void {
  ...
  const outPath = path.join(reportDir, `raw-ohlc-scanner-artifact-sweep-lunch-short-full-day-stable-field-miner-${Date.now()}.json`);
