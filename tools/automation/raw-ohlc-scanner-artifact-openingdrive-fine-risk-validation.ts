...
interface CliOptions {
  ...
  setupType: string;
  outDir: string;
  candidateFineRiskBucket: string;
...
export interface RawOhlcScannerArtifactOpeningDriveFineRiskValidationReport {
...
const DEFAULT_CANDIDATE_FINE_RISK_BUCKET = 'risk_24_to_32';
...
export function parseRawOhlcScannerArtifactOpeningDriveFineRiskValidationArgs(args = process.argv.slice(2)): CliOptions {
  ...
  return {
    ...
    outDir,
    candidateFineRiskBucket: readFlag(args, '--candidate-fine-risk-bucket') || DEFAULT_CANDIDATE_FINE_RISK_BUCKET,
...
function summarize(
  ...
  const matchingRows = rows.filter((row) => fineRiskBucket(row) === candidateFineRiskBucket);
...
function dateSummaries(
  ...
  return [...new Set(rows.map((row) => row.tradeDate))].sort().map((tradeDate) => {
    ...
    const matchingRows = dateRows.filter((row) => fineRiskBucket(row) === candidateFineRiskBucket);
...
function buildMarkdown(report: Omit<RawOhlcScannerArtifactOpeningDriveFineRiskValidationReport, 'markdown'>): string {
  return [
    '# Raw OHLC OpeningDrive Fine-Risk Validation',
    '',
...
export function buildRawOhlcScannerArtifactOpeningDriveFineRiskValidationReport(args: {
  reportDir: string;
  samebarSeparatorReportPath: string | null;
  ...
  validationPercent?: number;
  minValidationRows?: number;
}, generatedAt = new Date().toISOString()): RawOhlcScannerArtifactOpeningDriveFineRiskValidationReport {
  const candidateFineRiskBucket = args.candidateFineRiskBucket || DEFAULT_CANDIDATE_FINE_RISK_BUCKET;
  ...
  const allSummary = summarize('all', rows, candidateFineRiskBucket);
  const trainSummary = summarize('train', trainRows, candidateFineRiskBucket);
  const validationSummary = summarize('validation', validationRows, candidateFineRiskBucket);
  const blockers = [
  ...
  const validationDecision = !blockers.length &&
    trainSummary.matchingRows > 0 &&
    trainSummary.matchingLosses === 0 &&
    ...
    : 'not_validated';
  const base: Omit<RawOhlcScannerArtifactOpeningDriveFineRiskValidationReport, 'markdown'> = {
    reportType: 'raw_ohlc_scanner_artifact_openingdrive_fine_risk_validation',
    generatedAt,
    ...
    candidate: {
      feature: 'fineRiskBucket',
      featureValue: candidateFineRiskBucket,
    },
    splitPolicy: {
    ...
    dateSummaries: dateSummaries(rows, trainDates, validationDates, candidateFineRiskBucket),
    blockers,
    recommendations: blockers.length
      ? ['Do not use OpeningDrive fine-risk validation until the same-bar source report loads cleanly.']
      : validationDecision === 'validated_for_more_research'
        ? [
          'The frozen fine-risk candidate held up on the validation split as a research lead.',
          'Do not install scanner-visible behavior yet; next compare candidate overlap and selection priority against LONG risk_4_to_8.',
        ...
        : [
          'The frozen fine-risk candidate did not clear the validation gate.',
          'Keep OpeningDrive fine-risk selection research-only and mine richer proof-time structure before any live-facing change.',
          'No Discord, Supabase, NinjaTrader bridge, canExecute, entry/stop/target/risk, or trading-rule change is approved by this validation.',
        ],
  };
...
export function writeRawOhlcScannerArtifactOpeningDriveFineRiskValidationReport(
  report: RawOhlcScannerArtifactOpeningDriveFineRiskValidationReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `raw-ohlc-scanner-artifact-openingdrive-fine-risk-validation-${Date.now()}`;
...
export function runRawOhlcScannerArtifactOpeningDriveFineRiskValidationCli(args = process.argv.slice(2)): void {
  const options = parseRawOhlcScannerArtifactOpeningDriveFineRiskValidationArgs(args);
  ...
  const report = buildRawOhlcScannerArtifactOpeningDriveFineRiskValidationReport({
    ...
    candidateFineRiskBucket: options.candidateFineRiskBucket,
  ...
  const paths = writeRawOhlcScannerArtifactOpeningDriveFineRiskValidationReport(report, options.outDir);
  if (options.json) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    runRawOhlcScannerArtifactOpeningDriveFineRiskValidationCli();
  } catch (error) {
