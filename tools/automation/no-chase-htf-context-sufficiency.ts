...
export function buildNoChaseHtfContextSufficiencyReport(args: {
  ...
  const withoutRecommendationsAndMarkdown: Omit<NoChaseHtfContextSufficiencyReport, 'recommendations' | 'markdown'> = {
    ...
    summary: {
      ...
      afterLunchSufficient: BLANK_SLATE_MODE ? 0 : rows.filter((row) => row.setupType !== SetupType.NoSetup && row.sessionType === 'lunch' && row.sufficiency === 'sufficient').length,
      intradaySufficient: BLANK_SLATE_MODE ? 0 : rows.filter((row) => row.setupType !== SetupType.NoSetup && row.sessionType === 'morning' && row.sufficiency === 'sufficient').length,
