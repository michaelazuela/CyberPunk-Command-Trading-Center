...
type WordingRow = UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport['rows'][number];
...
export interface UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport {
...
function authority(): UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport['authority'] {
  return {
    readOnly: true,
    localOnly: true,
    researchOnly: true,
...
function buildMarkdown(report: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport, 'markdown'>): string {
  return [
    '# Unified Positive Held-Local Preview TurtleSoup Review Note Placement Simulation',
    '',
...
export function buildUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport(args: {
  reportDir: string;
  reviewNoteWordingProbePath: string | null;
  reviewNoteWordingProbeReport: UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport | null;
}, generatedAt = new Date().toISOString()): UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport {
  const wordingRows = args.reviewNoteWordingProbeReport?.rows || [];
  const rows = wordingRows.map(placementRow);
  const blockers = [
    !args.reviewNoteWordingProbePath ? 'missing TurtleSoup review-note wording probe path' : null,
    !args.reviewNoteWordingProbeReport ? 'missing TurtleSoup review-note wording probe report' : null,
    args.reviewNoteWordingProbeReport && args.reviewNoteWordingProbeReport.status !== 'pass' ? `TurtleSoup review-note wording probe status ${args.reviewNoteWordingProbeReport.status}` : null,
    wordingRows.length === 0 ? 'no TurtleSoup review-note wording rows found' : null,
  ...
  const base: Omit<UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport, 'markdown'> = {
    reportType: 'unified_positive_held_local_preview_turtlesoup_review_note_placement_simulation',
    generatedAt,
...
export function writeUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport(
  report: UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport,
  outDir = DEFAULT_REPORT_DIR,
): { jsonPath: string; markdownPath: string } {
  fs.mkdirSync(outDir, { recursive: true });
  const base = `unified-positive-held-local-preview-turtlesoup-review-note-placement-simulation-${Date.now()}`;
...
export function runUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationCli(args = process.argv.slice(2)): void {
  const outDir = readFlag(args, '--out-dir') || DEFAULT_REPORT_DIR;
  const reviewNoteWordingProbePath = readFlag(args, '--review-note-wording-probe') ||
    latestMatchingFile(outDir, /^unified-positive-held-local-preview-turtlesoup-review-note-wording-probe-\d+\.json$/);
  const report = buildUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport({
    ...
    reviewNoteWordingProbeReport: reviewNoteWordingProbePath && fs.existsSync(reviewNoteWordingProbePath)
      ? JSON.parse(fs.readFileSync(reviewNoteWordingProbePath, 'utf8')) as UnifiedPositiveHeldLocalPreviewTurtleSoupReviewNoteWordingProbeReport
      : null,
  });
  const paths = writeUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationReport(report, outDir);
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    runUnifiedPositiveHeldLocalPreviewTurtleSoupReviewNotePlacementSimulationCli();
  } catch (error) {
