...
import {
  buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport,
...
const installed: UnifiedPositiveHeldLocalPreviewSweepPenaltyInstalledScoreComparisonReport = {
  ...
  rows: [
    {
      ticketId: '2026-06-01-morning-TurtleSoup-LONG',
    ...
    {
      ticketId: '2026-06-02-lunch-TurtleSoup-LONG',
...
const separator = {
  ...
  topCautionBuckets: [{
    ...
    key: 'TurtleSoup|risk_extreme_over_15',
    rows: 3,
    winners: 1,
    losses: 2,
    unresolved: 0,
...
assert.equal(report.rows.find((row) => row.ticketId === '2026-06-01-morning-TurtleSoup-LONG')?.penaltyApplied, true);
assert.equal(report.rows.find((row) => row.ticketId === '2026-06-02-lunch-TurtleSoup-LONG')?.penaltyApplied, false);

const blocked = buildUnifiedPositiveHeldLocalPreviewTurtleSoupExtremeRiskRankSimulationReport({
  reportDir: 'reports',
...
console.log('unified positive held-local TurtleSoup extreme-risk rank simulation verified.');
